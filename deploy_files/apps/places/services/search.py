    ...
    def _fts_search(
        ...
        sql = text(f"""
            SELECT 
                ...
                m.processing_status,
                ts_rank(m.fts, websearch_to_tsquery('simple', :q)) AS rank_score
            FROM epx.places_search_mv m
            WHERE m.processing_status IN ('published','summarized','new')
              AND m.fts @@ websearch_to_tsquery('simple', :q)
              {area_sql}
    ...
    def _fts_search_with_categories(
        ...
        sql = text(f"""
            SELECT id, name, category, summary, tags_csv, lat, lng,
                   ...
                   ts_rank(fts, websearch_to_tsquery('simple', :tsq)) AS rank,
                   signals
            FROM epx.places_search_mv
            WHERE processing_status IN ('summarized','published')
              AND fts @@ websearch_to_tsquery('simple', :tsq)
              {where_categories}
              {where_area}
              {where_bbox}
