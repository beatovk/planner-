#!/usr/bin/env python3
"""
Добавление ночных заведений из CSV файла
"""
...
def add_nightlife_places(file_path: str):
    """Добавить ночные заведения из CSV"""
    db = SessionLocal()
    added_count = 0
    skipped_count = 0
    total_rows = 0
    ...
    try:
        with open(file_path, mode='r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                if not source_url:
                    source_url = f"nightlife_batch_02_{name.lower().replace(' ', '_').replace('(', '').replace(')', '').replace('—', '_').replace('.', '').replace(',', '')}"
                ...
                category = "bar"
                if "Club" in subcategory or "Club" in name:
                    category = "club"
                elif "Rooftop" in subcategory:
                    category = "rooftop"
                elif "Brewery" in subcategory:
                    category = "brewery"
            ...
            if places_to_add:
                ...
                print(f"✅ Добавлено {added_count} новых ночных заведений")
...
if __name__ == "__main__":
    csv_file_path = '../docs/places.csv/+Nightlife___Batch_02__verified__no_photos__24_venues_.csv'
    print(f"🚀 Добавление ночных заведений из {csv_file_path.split('/')[-1]}\n" + "="*60)
    add_nightlife_places(csv_file_path)
