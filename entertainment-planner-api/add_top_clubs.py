#!/usr/bin/env python3
"""
Добавление топ-клубов из CSV файла
"""
    ...
    try:
        with open(file_path, mode='r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                if not source_url:
                    base_url = f"top_clubs_batch_01_{name.lower().replace(' ', '_').replace('(', '').replace(')', '').replace('—', '_').replace('.', '').replace(',', '').replace('&', 'and')}"
                else:
                    base_url = f"top_clubs_{source_url}"
                
                # Убеждаемся что URL уникален
                source_url = base_url
                counter = 1
                ...
                # Все места из этого файла - клубы
                new_place = Place(
                    name=name,
            ...
            if places_to_add:
                ...
                print(f"✅ Добавлено {added_count} новых клубов")
...
if __name__ == "__main__":
    ...
    print(f"🚀 Добавление топ-клубов из {csv_file_path.split('/')[-1]}\n" + "="*60)
