...
from apps.places.services.search import SearchService, QueryBuilder
...
async def compose_rails(
    ...
    try:
        ...
        def _ensure_three_slots(req: ComposeRequest) -> list:
            ...
            if len(steps) < 3:
                qb = QueryBuilder()
                from_text = qb.build_slots(req.query or "")
                for sl in from_text:
                    if sl not in steps:
                        steps.append(sl)
                        if len(steps) == 3:
        ...
        def _suggest_rail(label: str, lat: Optional[float], lng: Optional[float]) -> Rail:
            ...
            items = [PlaceCard(
                ...
                picture_url=r.picture_url, website=None, phone=None,
...
async def get_rails(
    ...
    # Free-text query mode (slotter)
        ...
        # Check if slotter should be used
        if not should_use_slotter(q):
            logger.info(f"Slotter disabled for query: {q}")
            # Fallback to default behavior
            return ComposeResponse(rails=[], processing_time_ms=0.0, cache_hit=False)
        
        # Shadow mode logging
        if should_log_slotter(q):
            logger.info(f"Shadow mode: Processing query '{q}' with slotter")
        
        # A/B testing
        ab_test = should_ab_test_slotter(q)
        if ab_test:
            logger.info(f"A/B test: Query '{q}' selected for slotter testing")
        ...
        # Generate new result using slotter
        result = await _compose_slotter_rails(q, area, user_lat, user_lng, quality_only, db)
        
        # Shadow mode logging
        if should_log_slotter(q):
            logger.info(f"Shadow mode: Generated {len(result.rails)} rails for query '{q}' in {result.processing_time_ms:.2f}ms")
            for i, rail in enumerate(result.rails):
                logger.info(f"Shadow mode: Rail {i+1}: {rail.label} ({len(rail.items)} items)")
