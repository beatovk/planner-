...
class TimeOutAdapter:
    ...
    def parse_list_page(self, list_url: str) -> List[Dict]:
        ...
        # Метод 5: Поиск по заголовкам h3 (специально для TimeOut)
        places.extend(self._extract_from_headers(soup))
    ...
    def _extract_original_links(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        restaurant_links = main_content.find_all('a', href=re.compile(r'/bangkok/(restaurants|bars)/'))
        ...
        for link in restaurant_links:
            ...
            if not re.match(r'^\d+\.\s+', title) and not self._is_valid_place_name(title):
                continue
    ...
    def _is_valid_place_name(self, title: str) -> bool:
        ...
        service_words = [
            ...
            'width', 'height', 'path', 'circle', 'rect', 'polygon'
        ]
        ...
        technical_patterns = [
            ...
            # r'^[A-Z]{2,}',     # ВСЕ ЗАГЛАВНЫЕ - убираем, это валидные названия
            r'^[a-z]{2,}$',     # все строчные (только если полностью строчные)
        ...
        place_patterns = [
            ...
            r'^[A-Z][a-zA-Z\s]+$',   # Название с заглавной буквы (включая пробелы)
            r'^[A-Z]+[a-z]*\s+[A-Z][a-z]+',  # "BEAM Cube", "12 x 12" и т.д.
            r'^[A-Z][a-z]+$',        # Одно слово с заглавной буквы (Midori, Decommune)
            r'^[A-Z][a-z]+\s+[A-Z][a-z]+',  # Два слова с заглавными буквами
        ]
        
        # Если не технический паттерн, то это валидное название места
        return True
    ...
    def _extract_restaurant_links(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        links = soup.find_all('a', href=re.compile(r'/bangkok/(restaurants|bars)/'))
    ...
    def _extract_from_headers(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        """Поиск мест по заголовкам h3, h2 и другим структурам"""
        places = []
        
        # Метод 1: Специальный метод для TimeOut структуры
        timeout_places = self._extract_from_timeout_structure(soup)
        places.extend(timeout_places)
        
        # Метод 2: Извлечение из h3 заголовков
        h3_places = self._extract_from_h3_headers(soup)
        places.extend(h3_places)
        
        # Метод 3: Извлечение из h2 заголовков
        h2_places = self._extract_from_h2_headers(soup)
        places.extend(h2_places)
        
        # Метод 4: Извлечение из div контейнеров
        div_places = self._extract_from_div_containers(soup)
        places.extend(div_places)
        
        return places
    
    def _extract_from_timeout_structure(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        """Специальный метод для извлечения из TimeOut структуры"""
        places = []
        
        # Ищем все ссылки с h3 заголовками
        h3_links = soup.find_all('a')
        ...
        for link in h3_links:
            ...
            # Ищем описание в следующем блоке после ссылки
            description = self._find_timeout_description(link)
    ...
    def _find_timeout_description(self, link_elem) -> Optional[str]:
        """Поиск описания в TimeOut структуре"""
        # Ищем следующий элемент после ссылки
        next_elem = link_elem.find_next_sibling()
        if next_elem:
            # Ищем параграфы в следующем элементе
            p_tags = next_elem.find_all('p')
            for p in p_tags:
        ...
        # Ищем в следующем div с классом
        next_div = link_elem.find_next('div')
        if next_div:
    ...
    def _extract_from_h3_headers(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        """Извлечение мест из заголовков h3"""
        places = []
        ...
        for h3 in h3_headers:
            ...
            # Ищем описание в следующем параграфе
            description = self._find_description_after_header(h3)
            
            # Ищем URL детальной страницы
            detail_url = self._find_place_url(h3)
    ...
    def _extract_from_h2_headers(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        """Извлечение мест из заголовков h2"""
        places = []
        ...
        for h2 in h2_headers:
            ...
            # Ищем описание в следующем параграфе
            description = self._find_description_after_header(h2)
            
            # Ищем URL детальной страницы
            detail_url = self._find_place_url(h2)
    ...
    def _extract_from_div_containers(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        """Извлечение мест из div контейнеров"""
        places = []
        
        # Ищем div с классами, содержащими названия мест
        div_containers = soup.find_all('div', class_=re.compile(r'(place|venue|location|item|card)', re.I))
        
        for div in div_containers:
            # Ищем заголовки внутри div
            ...
            # Ищем описание
            description = self._find_description_in_container(div)
            
            # Ищем URL детальной страницы
            detail_url = self._find_place_url(div)
    ...
    def _find_description_after_header(self, header_elem) -> Optional[str]:
        """Поиск описания после заголовка"""
        # Ищем в следующем параграфе
        next_p = header_elem.find_next('p')
        if next_p:
            description = next_p.get_text().strip()
            if len(description) > 20 and len(description) < 500:
                return description
        
        # Ищем в следующем div
        next_div = header_elem.find_next('div')
        if next_div:
            description = next_div.get_text().strip()
            if len(description) > 20 and len(description) < 500:
                return description
        
        return None
    
    def _find_description_in_container(self, container) -> Optional[str]:
        """Поиск описания в контейнере"""
        # Ищем параграфы
        ...
        desc_div = container.find('div', class_=re.compile(r'(desc|text|content|summary)', re.I))
        if desc_div:
